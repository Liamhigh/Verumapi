...
import { UserIcon, PaperclipIcon, ShieldCheckIcon, DownloadIcon } from './Icons';
  ...
  return (
    <div className={`flex items-start gap-4 my-6 ${!isModel ? 'flex-row-reverse' : ''}`}>
      <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center ${isModel ? 'bg-slate-700' : 'bg-sky-800'}`}>
        {isModel ? <img src="/main_logo.png" alt="Verum Omnis Logo" className="h-5 w-5 object-contain" /> : <UserIcon className="h-5 w-5 text-sky-200" />}
      </div>
